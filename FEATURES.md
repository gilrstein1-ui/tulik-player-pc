...
**Round 5 (C6) — CODE-COMPLETE, NOT YET BUILT (blocked on GitHub Actions billing):** "Start something" idle panel in the Lyrics tile — when nothing is playing the tile offers 🎲 Surprise / ⭐ Top tracks / 🕘 Recently played / 📃 Playlists. Committed (39b66e8) but the CI run failed to start: _"The job was not started because recent account payments have failed or your spending limit needs to be increased."_ The Pi can't cross-compile a Windows exe, so **build 27 (theme + screenshots) is the live native build**; C6 ships on the next CI run after the GitHub billing/spending-limit is resolved.
